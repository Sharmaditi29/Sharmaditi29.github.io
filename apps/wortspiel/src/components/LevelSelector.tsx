    ...
    <section className="border-t border-line/80 pt-4">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="min-w-0">
          ...
          <h2 className="mt-1 font-display text-lg font-bold text-ink">Keep it beginner-simple.</h2>
        </div>
        ...
        {levels.map(({ level, available, subtitle }) => {
          ...
          return (
            <button
              ...
              className={`min-w-0 rounded-[18px] border p-3.5 text-left transition ${
                available
                  ? isSelected
            ...
            >
              <div className="flex items-center justify-between">
                <span className="font-display text-lg font-bold">{level}</span>
                {!available && (
              ...
              <p className={`mt-2 text-sm ${isSelected && available ? 'text-paper/80' : 'text-notebook'}`}>
                {subtitle}
              </p>
            </button>
          )
