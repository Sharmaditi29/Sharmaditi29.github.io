    ...
    <section className="border-t border-line/80 pt-3">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <h2 className="mt-1 font-display text-base font-bold text-ink">Keep it beginner-simple.</h2>
        </div>
        ...
        {levels.map(({ level, available, subtitle }) => {
          ...
          return (
            <button
              ...
              className={`min-w-0 rounded-[14px] border px-3 py-2.5 text-left transition ${
                available
                  ? isSelected
            ...
            >
              ...
              <p className={`mt-1 text-[13px] ${isSelected && available ? 'text-paper/80' : 'text-notebook'}`}>
                {subtitle}
              </p>
            </button>
          )
